...
export class CourseListComponent {
  ...
  @Output() courseToggle = new EventEmitter<{courseId: number, selected: boolean}>();
  ...
  updateSelectAllState(): void {
    this.selectAllChecked = this.courses.length > 0 && 
  ...
  onSort(field: string): void {
    const newOrder = this.sortBy === field && this.sortOrder === 'asc' ? 'desc' : 'asc';

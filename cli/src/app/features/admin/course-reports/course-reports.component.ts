...
export class CourseReportsComponent implements OnInit, OnDestroy {
  ...
  selectedReportType: 'comprehensive' | 'performance' | 'enrollment' | 'revenue' | 'reviews' = 'comprehensive';
  ...
  loadReportTypes() {
    ...
    this.adminService.getCourseReportTypes()
      .pipe(
  ...
  onReportTypeChange(type: string) {
    if (type === 'comprehensive' || type === 'performance' || type === 'enrollment' || type === 'revenue' || type === 'reviews') {
  ...
  generateReport() {
    ...
    this.adminService.generateCourseReport(params)
      .pipe(
        takeUntil(this.destroy$),
      ...
      .subscribe({
        next: (data) => {
          if (this.selectedFormat === 'csv') {
            ...
            link.href = url;
            link.download = `course-report-${this.selectedReportType}-${Date.now()}.csv`;
            link.click();
    ...
    this.adminService.generateCourseReport(params)
      ...
      .subscribe({
        next: (data) => {
          ...
          const link = document.createElement('a');
          link.href = url;
          link.download = `course-report-${this.selectedReportType}-${Date.now()}.${exportFormat}`;
          link.click();
          window.URL.revokeObjectURL(url);
        },
        error: (error) => {
  ...
  formatCurrency(value: unknown): string {
    ...
    if (num >= 1000) {
      return (num / 1000).toFixed(1) + 'K VNĐ';
    }
